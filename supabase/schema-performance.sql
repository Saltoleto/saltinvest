...
-- =========================================================
--  PERFORMANCE (Índices e ajustes)
--  Observação:
--  - Supabase já cria índices automaticamente para:
--      * PRIMARY KEY
--      * UNIQUE constraints
--      * Alguns índices internos do sistema
--  - Os índices abaixo focam nos principais padrões de consulta do app:
--      * listas filtradas por usuario_id
--      * plano mensal por mes_referencia
--      * agregações por categoria / instituição / liquidez
--      * busca por nome (ILIKE)
-- =========================================================

-- (Opcional, recomendado) Suporte a busca rápida por texto (ILIKE) usando trigram.
create extension if not exists pg_trgm;

-- -------------------------
--  Índices para listas por usuário (ORDER BY criado_em / aportado_em)
-- -------------------------
create index if not exists idx_categorias_ativos_usuario_criado_em
  on public.categorias_ativos (usuario_id, criado_em desc);

create index if not exists idx_instituicoes_financeiras_usuario_criado_em
  on public.instituicoes_financeiras (usuario_id, criado_em desc);

create index if not exists idx_objetivos_usuario_data_alvo
  on public.objetivos (usuario_id, data_alvo);

create index if not exists idx_objetivos_usuario_participa_plano_data_alvo
  on public.objetivos (usuario_id, participa_plano_mensal, data_alvo);

create index if not exists idx_aplicacoes_usuario_criado_em
  on public.aplicacoes (usuario_id, criado_em desc);

create index if not exists idx_aplicacoes_usuario_status_criado_em
  on public.aplicacoes (usuario_id, status, criado_em desc);

-- Índice parcial para a listagem mais comum (aplicações ativas)
create index if not exists idx_aplicacoes_usuario_ativas_criado_em
  on public.aplicacoes (usuario_id, criado_em desc)
  where status = 'ATIVA';

create index if not exists idx_aportes_usuario_aportado_em
  on public.aportes (usuario_id, aportado_em desc);

create index if not exists idx_resgates_usuario_resgatado_em
  on public.resgates_aplicacoes (usuario_id, resgatado_em desc);

-- -------------------------
--  Índices para JOINs / filtros por chaves estrangeiras
-- -------------------------
-- Aportes: filtros por meta (objetivo), por aplicação, e ligação com parcela do mês
create index if not exists idx_aportes_usuario_objetivo
  on public.aportes (usuario_id, objetivo_id);

create index if not exists idx_aportes_usuario_aplicacao
  on public.aportes (usuario_id, aplicacao_id);

create index if not exists idx_aportes_parcela_objetivo
  on public.aportes (parcela_objetivo_id);

-- Parcelas do objetivo: plano mensal por mês e usuário
create index if not exists idx_parcelas_usuario_mes_referencia
  on public.parcelas_objetivo (usuario_id, mes_referencia);

create index if not exists idx_parcelas_usuario_status_mes
  on public.parcelas_objetivo (usuario_id, status, mes_referencia);

-- Aplicações: agregações no dashboard (concentração)
create index if not exists idx_aplicacoes_usuario_categoria
  on public.aplicacoes (usuario_id, categoria_ativo_id);

create index if not exists idx_aplicacoes_usuario_instituicao
  on public.aplicacoes (usuario_id, instituicao_financeira_id);

create index if not exists idx_aplicacoes_usuario_liquidez
  on public.aplicacoes (usuario_id, liquidez);

-- Resgates: histórico por aplicação
create index if not exists idx_resgates_aplicacao
  on public.resgates_aplicacoes (aplicacao_id);

-- Política de alocação: itens consultados para montar alvo
create index if not exists idx_politicas_usuario_criado_em
  on public.politicas_alocacao (usuario_id, criado_em desc);

create index if not exists idx_politica_itens_politica
  on public.politicas_alocacao_itens (politica_alocacao_id);

-- -------------------------
--  Busca por texto (ILIKE / contains) - trigram
--  Use em telas com "Buscar" (nome)
-- -------------------------
create index if not exists idx_aplicacoes_nome_trgm
  on public.aplicacoes using gin (lower(nome) gin_trgm_ops);

create index if not exists idx_objetivos_nome_trgm
  on public.objetivos using gin (lower(nome) gin_trgm_ops);

create index if not exists idx_categorias_nome_trgm
  on public.categorias_ativos using gin (lower(nome) gin_trgm_ops);

create index if not exists idx_instituicoes_nome_trgm
  on public.instituicoes_financeiras using gin (lower(nome) gin_trgm_ops);

-- -------------------------
--  Estatísticas
--  (Ajuda o planner após carga inicial / migração)
-- -------------------------
analyze public.categorias_ativos;
analyze public.instituicoes_financeiras;
analyze public.objetivos;
analyze public.parcelas_objetivo;
analyze public.aplicacoes;
analyze public.aportes;
analyze public.resgates_aplicacoes;
analyze public.politicas_alocacao;
analyze public.politicas_alocacao_itens;
